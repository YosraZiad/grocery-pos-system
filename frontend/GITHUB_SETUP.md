...
3. اسم الـ Repository: `grocery-pos-frontend`
...
```bash
git remote add origin git@github.com:YOUR_USERNAME/grocery-pos-frontend.git
```
...
2. **مجلد `node_modules/`** لن يُرفع (موجود في `.gitignore`)
3. **مجلد `dist/`** لن يُرفع (موجود في `.gitignore`)
